...
use ratatui::{backend::TestBackend, Terminal, text::Span, style::{Color, Style}};
...
fn test_status_bar_snapshot() {
    ...
    // Verify context
    assert!(buffer.content.iter().any(|c| c.symbol() == "C" && c.fg == Color::Gray));
    // Verify mode
    assert!(buffer.content.iter().any(|c| c.symbol() == "N" && c.fg == Color::Cyan));
...
fn test_dialog_snapshot() {
    ...
    // Verify title
    assert!(buffer.content.iter().any(|c| c.symbol().contains('T')), "Title character 'T' not found");
    // Verify content
    assert!(buffer.content.iter().any(|c| c.symbol().contains('A')), "Content character 'A' not found");
    // Verify buttons (they are rendered as "[ Yes ]" and "  No  " when first is selected)
    assert!(buffer.content.iter().any(|c| c.symbol().contains('Y')), "Button character 'Y' not found"); 
    assert!(buffer.content.iter().any(|c| c.symbol().contains('N')), "Button character 'N' not found"); 
}
...
fn test_tab_bar_snapshot() {
    ...
    terminal.draw(|f| {
        let tab_bar = TabBar::new(vec!["Issues", "Labels", "Database"])
            .selected(1);
    ...
    assert!(buffer.content.iter().any(|c| c.symbol().contains('L') && c.fg == Color::Yellow));
}
