...
async fn test_concurrent_modifications() {
    ...
    let handle = thread::spawn(move || {
        for i in 0..10 {
            ...
            let status = Command::new("bd")
                .args(["create", "--title", &title, "--type", "task", "--priority", "P2"])
    ...
    // Final verification
    let final_issues = harness.client.list_issues(None, None).await.expect("Final read failed");
    assert!(final_issues.len() >= 10, "Expected at least 10 issues, found {}", final_issues.len());
}
